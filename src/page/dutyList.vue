...
<script>
	...
	export default {
		...
		mounted() {
			let that = this;
			this.$ajax.post(
				'http://localhost:9090/DS/selectDutyNewA.do'
			).then(function (res) {
				that.data4 = res.data.children;
			})
...
//			        children.splice(index, 1);
			...
			/*renderContent(h, { node, data, store }) {
				return ('<div style="width:100%;height:100%;display: flex; align-items: center; justify-content: center;">' +
                    '<div style="flex-grow:4;flex-basis: 0%;display: flex;justify-content: start;">'+(node.label)+'</div></div>')

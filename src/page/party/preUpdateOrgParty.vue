<template>
    ...
    >
        <el-form-item prop='PARTY_CODE_' label="机构编码:"
            ...
            <el-input v-model="updateOrgParty.PARTY_CODE_"></el-input>
        </el-form-item>
        <el-form-item prop='PARTY_NAME_' label="机构名称:"
            ...
            <el-input v-model="updateOrgParty.PARTY_NAME_"></el-input>
        </el-form-item>
        <el-form-item prop='ADDR_' label="地址:"
                      :rules="[{ type: 'string', max: 100, message: '地址长度不能超过1000' ,trigger: 'blur' }]">
            <el-input type="textarea" v-model="updateOrgParty.ADDR_"></el-input>
        </el-form-item>
        <el-form-item prop='MEMO_' label="备注:"
                      :rules="[{ type: 'string', max: 100, message: '备注长度不能超过1000' ,trigger: 'blur' }]">
            <el-input type="textarea" v-model="updateOrgParty.MEMO_"></el-input>
        </el-form-item>
        <el-form-item class="text_right">
            <el-button type="primary" @click='_updateOrgParty("updateOrgParty")'>保 存</el-button>
...
<script>
	...
	export default {
		...
		methods: {
			_updateOrgParty(updateOrgParty) {
				this.$refs[updateOrgParty].validate((valid) => {
					if (valid) {//表单数据验证完成之后，提交数据;
						let formData = this[updateOrgParty];
						let that = this;
						axios({
							method: 'POST',
							header: '',
							...
							data: {
								PARTY_ID_: this.updateOrgParty.PARTY_ID_,
							...
							if (res.data.success) {
								...
								that.updateOrgParty.show = false;
							} else {
								that.$message.error(res.data.message);
							}
						}).catch(function (err) {
							/*请求失败*/
							that.$message.error('编辑失败请重试');
						})
			...
			_close() {
				this._closePreUpdateOrgPartyDialog();
			}
		}
	}
</script>

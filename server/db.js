...
//mongoose.connect('mongodb://localhost:27017/DS',{useMongoClient: true});

// 为这次连接绑定事件
const ds = mongoose.connection;
ds.on('error', console.error.bind(console, '连接错误'));
ds.once('open', function() {console.log(new Date+'  http:localhost:80连接成功')});
ds.on("disconnected", function () { console.log("MongoDB connected disconnected.")});
